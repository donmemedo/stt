...
class FLASH_ShareA_FFConvM(nn.Module):
    ...
    def cal_attention(self, x, quad_q, lin_q, quad_k, lin_k, v, u, mask=None):
        ...
        if padding > 0:
            quad_q, quad_k, lin_q, lin_k, v, u = map(lambda t: F.pad(t, (0, 0, 0, padding), value=0.),
        ...
        quad_q, quad_k, lin_q, lin_k, v, u = map(lambda t: rearrange(t, 'b (g n) d -> b g n d', n=self.group_size),
                                                 (quad_q, quad_k, lin_q, lin_k, v, u))
...
class FLASHTransformer_DualA_FSMN(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList([FLASH_ShareA_FFConvM(dim=dim, group_size=group_size, query_key_dim=query_key_dim,
                                                          ...
                                                          norm_class=norm_class, shift_tokens=shift_tokens) for _ in
                                     range(depth)])
